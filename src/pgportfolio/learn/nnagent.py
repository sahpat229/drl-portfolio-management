...
class NNAgent:
    def __init__(self, config, restore_dir=None, device="cpu"):
        ...
        self.y_predict = tf.reshape(self.__net.y_predict,
                                    shape=[-1, self.__coin_number])
        print("__additional_loss")
        self.__additional_loss = tf.reduce_mean(tf.reduce_sum(tf.square(self.y_predict - self.__y[:, 0, :]), axis=1))
        self.__loss += 1e0*self.__additional_loss
